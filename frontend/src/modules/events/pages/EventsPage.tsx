  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-[#e2e8f0]">Eventos</h1>
          <p className="text-[#94a3b8] text-sm mt-1">Gestiona los eventos deportivos</p>
        </div>
        <button
      ...
      <div className="flex gap-4">
        <input
          ...
          className="flex-1 bg-[#171923] border border-white/5 rounded-lg py-2.5 px-4 text-[#e2e8f0] placeholder-[#94a3b8] focus:border-[#f97316] focus:outline-none"
        />
        <select
          value={statusFilter}
          onChange={e => setStatusFilter(e.target.value)}
          className="bg-[#171923] border border-white/5 rounded-lg py-2.5 px-4 text-[#e2e8f0] focus:border-[#f97316] focus:outline-none w-40"
        >
          ...
          <tbody>
            {filteredEvents?.map((event) => (
              <tr key={event.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors">
                <td className="py-3 px-4 text-[#e2e8f0] font-medium">{event.name}</td>
